...
import { chunkArray } from '@/lib/instagram-followers-parse'
...
export async function createInstagramFollowerUploadJob(
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      progress_index: 0,
      total_parsed: options.usernames.length,
      updated_at: nowIso,
    })
    .select('*')
    .single()
...
export async function processInstagramFollowerUploadJob(
  ...
  const usernames = Array.isArray(claimed.usernames)
